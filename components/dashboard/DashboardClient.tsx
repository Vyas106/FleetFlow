...
import { Search, Truck, AlertTriangle, PackageSearch, ActivitySquare, Plus, Car, Filter } from "lucide-react"
...
export default function DashboardClient({ stats }: DashboardClientProps) {
    ...
    const filteredTrips = useMemo(() => {
        return stats.activeTrips.filter((trip: any) => {
            const matchesSearch = searchQuery === "" ||
                trip.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
                trip.vehicle.licensePlate.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    }, [stats.activeTrips, searchQuery, statusFilter, typeFilter])
            ...
            <div className="flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center p-1 rounded-2xl bg-background/40 backdrop-blur-sm border border-border/40 shadow-sm p-4">
                <div className="flex flex-1 w-full sm:w-auto items-center gap-2">
                    ...
                    <Select value={typeFilter} onValueChange={setTypeFilter}>
                        <SelectTrigger className="w-[130px] hidden lg:flex rounded-full bg-background border-border/50">
                    ...
                    <Select value={statusFilter} onValueChange={setStatusFilter}>
                        <SelectTrigger className="w-[130px] hidden md:flex rounded-full bg-background border-border/50">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card className="glass-card hover:border-primary/30 transition-all duration-300 group">
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Currently on mission</p>
                    </CardContent>
                </Card>
                                ...
                                <TableRow className="hover:bg-transparent">
                                    <TableCell colSpan={4} className="h-48 text-center">
                                        <div className="flex flex-col items-center justify-center text-muted-foreground gap-3">
                                            ...
                                            <p>{searchQuery ? "No missions match your search." : "No active missions at this time."}</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : (
